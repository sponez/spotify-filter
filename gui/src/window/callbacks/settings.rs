...
use domain::ports::ports_in::{events::AppRequest, settings::models::SaveSettingsCommand};
...
pub fn setup_save_settings_callback(window: &AppWindow, tx: Sender<AppRequest>) {
    ...
    window.on_save_settings(move || {
        if let Some(w) = w.upgrade() {
            ...
            let filter_target =
                slint_to_target_view(w.get_filter_target_type(), w.get_filter_playlist_index());
